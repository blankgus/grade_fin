...
# ============================================
# ============================================
# ABA GRADE POR PROFESSOR (COM VISUALIZAÇÃO EM CALENDÁRIO)
# ============================================
with abas[6]:
    ...
    if not st.session_state.get('aulas'):
        ...
        if professor_selecionado:
            ...
            if not aulas_professor:
                ...
                # ============================================
                # VISUALIZAÇÃO EM CALENDÁRIO (COMO NAS TURMAS)
                # ============================================
                st.subheader("📅 Visualização em Calendário")
                
                # Agrupar aulas por turma para exibição
                ...
                for turma_nome in sorted(turmas_professor):
                    st.write(f"#### 🎒 Turma: {turma_nome}")
                    
                    # Filtrar aulas desta turma deste professor
                    aulas_turma = [a for a in aulas_professor if obter_turma_aula(a) == turma_nome]
                    
                    # Determinar segmento e períodos
                    segmento_turma = obter_segmento_turma(turma_nome)
                    ...
                    # CSS para estilizar a tabela (mesmo estilo das turmas)
                    st.markdown("""
                    <style>
                    .grade-table-prof {
                        width: 100%;
                    ...
                    .grade-table-prof th, .grade-table-prof td {
                        border: 1px solid #ddd;
                        padding: 8px;
                        text-align: center;
                        vertical-align: middle;
                    ...
                    .grade-table-prof th {
                        background-color: #9C27B0;  /* Roxo para professores */
                        color: white;
                        font-weight: bold;
                    }
                    .aula-cell-prof {
                    ...
                    # Construir tabela HTML
                    table_html = """<table class='grade-table-prof'>
                    <tr>
                    <th>Horário REAL</th>
                    <th>Segunda</th>
                    <th>Terça</th>
                    ...
                    for periodo in periodos:
                        horario_real = obter_horario_real(turma_nome, periodo)
                        table_html += f"<tr><td class='horario-cell-prof'><strong>{horario_real}</strong><br><small>(Período {periodo}º)</small></td>"
                        ...
                        for dia in dias_ordenados:
                            ...
                            if aula_encontrada:
                                # Obter dados da aula
                                disciplina = str(obter_disciplina_aula(aula_encontrada)).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                                turma = str(obter_turma_aula(aula_encontrada)).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                                
                                # Encontrar cor da disciplina
                                cor_fundo = "#f3e5f5"  # Default roxo claro
                                cor_fonte = "#4A148C"  # Default roxo escuro
                                for disc in st.session_state.disciplinas:
                                    if disc.nome == disciplina:
                                        if hasattr(disc, 'cor_fundo') and disc.cor_fundo:
                                            cor_fundo = disc.cor_fundo
                                ...
                                table_html += f"""<td class='aula-cell-prof' style='background-color: {cor_fundo}; color: {cor_fonte};'>
                                <div class='disciplina-prof'>{disciplina}</div>
                                <div class='turma-prof'>{turma}</div>
                                </td>"""
                            else:
                                table_html += f"<td class='livre-cell-prof'>-</td>"
                        
                        table_html += "</tr>"
                        
                        # Adicionar linha do intervalo no lugar correto
                        if segmento_turma == "EM" and periodo == 3:
                            table_html += """<tr class='intervalo-row-prof'>
                            <td colspan='6'>🕛 INTERVALO: 09:30 - 09:50</td>
                            </tr>"""
                        elif segmento_turma == "EF_II" and periodo == 2:
                            table_html += """<tr class='intervalo-row-prof'>
                            <td colspan='6'>🕛 INTERVALO: 09:30 - 09:50</td>
                            </tr>"""
                    ...
                    # Resumo da turma
                    st.caption(f"📚 {len(aulas_turma)} aulas nesta turma | Segmento: {segmento_turma}")
                    st.markdown("---")
                
                # ============================================
                # GRADE COMPLETA DO PROFESSOR (TODAS AS AULAS JUNTAS)
                # ============================================
                st.subheader("📋 Grade Completa do Professor")
                
                # Criar tabela com todas as aulas do professor (todas turmas juntas)
                # Para isso, precisamos determinar os horários máximos que ele pode ter
                
                # Encontrar o maior número de períodos entre todas as turmas que ele atende
                max_periodos = 0
                for turma_nome in turmas_professor:
                    segmento = obter_segmento_turma(turma_nome)
                    periodos_turma = 7 if segmento == "EM" else 5
                    if periodos_turma > max_periodos:
                        max_periodos = periodos_turma
                
                st.write(f"#### 📅 Calendário Completo - Todas as Turmas")
                
                # CSS para a tabela completa
                st.markdown("""
                <style>
                .grade-completa-table {
                    width: 100%;
                    border-collapse: collapse;
                    margin: 20px 0;
                    font-size: 13px;
                }
                .grade-completa-table th, .grade-completa-table td {
                    border: 1px solid #ddd;
                    padding: 6px;
                    text-align: center;
                    vertical-align: middle;
                }
                .grade-completa-table th {
                ...
                # Construir tabela completa
                dias_completos = ["segunda", "terca", "quarta", "quinta", "sexta"]
                periodos_completos = list(range(1, max_periodos + 1))
                
                table_completa_html = """<table class='grade-completa-table'>
                <tr>
                <th>Horário</th>"""
                
                # Cabeçalho dos dias
                for dia in dias_completos:
                    table_completa_html += f"<th>{dia.capitalize()}</th>"
                table_completa_html += "</tr>"
                
                # Para cada período
                for periodo in periodos_completos:
                    table_completa_html += f"<tr><td class='header-cell'><strong>Período {periodo}º</strong></td>"
                    ...
                    for dia in dias_completos:
                        ...
                        if aulas_no_horario:
                            # Se tiver mais de uma aula (superposição - PROBLEMA!)
                            if len(aulas_no_horario) > 1:
                                table_completa_html += f"""<td class='aula-completa-cell' style='background-color: #ffcdd2; color: #c62828;'>
                                <div class='disciplina-completa'>⚠️ CONFLITO!</div>
                                <div class='turma-completa'>{len(aulas_no_horario)} aulas</div>
                                </td>"""
                            else:
                                # Apenas uma aula
                                ...
                                table_completa_html += f"""<td class='aula-completa-cell' style='background-color: {cor_fundo}; color: {cor_fonte};'>
                                <div class='disciplina-completa'>{disciplina}</div>
                                <div class='turma-completa'>{turma}</div>
                                <div class='horario-completa'>{horario_real.split(' - ')[0]}</div>
                                </td>"""
                        else:
                            table_completa_html += "<td class='vazio-cell'>-</td>"
                    
                    table_completa_html += "</tr>"
                    
                    # Adicionar intervalo se for período 3 (EM) ou 2 (EF II)
                    if periodo == 3 and max_periodos >= 3:
                        table_completa_html += """<tr style='background-color: #f3e5f5;'>
                        <td colspan='6' style='text-align: center; font-weight: bold; color: #7B1FA2;'>
                        🕛 INTERVALO: 09:30 - 09:50
                        </td>
                        </tr>"""
                
                table_completa_html += "</table>"
                
                # Mostrar tabela completa
                st.markdown(table_completa_html, unsafe_allow_html=True)
                
                # Legenda
                st.caption("🎨 **Legenda:** Cores representam as disciplinas | ⚠️ Vermelho indica superposição de horário")
                
                # ============================================
                # LISTA DETALHADA (mantida para referência)
                # ============================================
                with st.expander("📊 Lista Detalhada das Aulas"):
                    ...
                    with col1:
                        # Aulas por dia
                        aulas_por_dia = df_professor['Dia'].value_counts()
                        st.write("**Aulas por dia:**")
                        for dia, count in aulas_por_dia.items():
                            st.write(f"- {dia}: {count} aulas")
                    
                    with col2:
                        # Aulas por turma
                        aulas_por_turma = df_professor['Turma'].value_counts()
                        st.write("**Aulas por turma:**")
                        for turma, count in aulas_por_turma.head(5).items():
                            st.write(f"- {turma}: {count} aulas")
                    
                    with col3:
                        # Aulas por disciplina
                        aulas_por_disc = df_professor['Disciplina'].value_counts()
                        st.write("**Aulas por disciplina:**")
                        for disc, count in aulas_por_disc.items():
                            st.write(f"- {disc}: {count} aulas")

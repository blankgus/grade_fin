...
ALGORITMOS_DISPONIVEIS = True
try:
    from simple_scheduler_final import SimpleGradeHoraria  # ← MUDAR AQUI
except ImportError:
    ...
    class SimpleGradeHoraria:
        ...
        def gerar_grade(self):
            st.error("❌ Algoritmo não disponível")
            return []
try:
...
# ============================================
# ALGORITMO AVANÇADO PARA COMPLETAR GRADES (mantido igual)
# ============================================
...
# ============================================
# ABA GERAR GRADE (ATUALIZADA COM VERIFICAÇÃO DE HORÁRIOS REAIS)
# ============================================
with abas[5]:
    ...
    if not st.session_state.get('turmas') or not st.session_state.get('professores'):
        ...
        with st.spinner("Gerando grade horária inteligente... (pode levar 10–60 segundos dependendo da quantidade de turmas)"):
            try:
                # Instancia o scheduler melhorado
                ...
                # Gera a grade → aqui já aparece progresso + relatório detalhado automaticamente
                novas_aulas = scheduler.gerar_grade()
                
                if novas_aulas:
                    st.session_state.aulas = novas_aulas
                    
                    st.success(f"Grade gerada com sucesso! {len(novas_aulas)} aulas alocadas.")
                    
                    # Salva automaticamente (se sua função existir)
                    if 'salvar_tudo' in globals() and callable(salvar_tudo):
                        if salvar_tudo():
                            st.success("✅ Dados salvos automaticamente no banco!")
                        else:
                            st.warning("Grade gerada, mas falhou ao salvar automaticamente.")
                    
                    # Força atualização da interface
                    st.rerun()
                else:
                    st.warning("Nenhuma aula alocada. Veja o relatório acima para entender os bloqueios.")
            
            except Exception as e:
                st.error(f"Erro crítico durante a geração da grade: {str(e)}")
                import traceback
                st.code(traceback.format_exc(), language="python")
                st.info("Tente resetar os dados ou verificar professores/disciplinas cadastrados.")
            ...
            if not turmas_filtradas:
                ...
                with st.spinner(f"Gerando grade para {grupo_texto}..."):
                    try:
                        ...
                        # ============================================
                        # ETAPA 2: VERIFICAR SUPERPOSIÇÕES DE PROFESSOR (CRÍTICO!) - COM HORÁRIOS REAIS
                        # ============================================
                        ...
                        if superposicoes:
                            ...
                            st.error("⚠️ **Um professor NÃO PODE dar aula em dois lugares ao mesmo horário REAL!**")
                            
                            # Mostrar exemplos com horários REAIS
                            with st.expander("📋 Ver Superposições Críticas (Horários REAIS)", expanded=True):
                                for i, sup in enumerate(superposicoes[:5]):
                                    st.write(f"**Professor {sup['professor']}**: {len(sup['aulas'])} aulas no mesmo horário REAL ({sup['dia']}, {sup['horario_real']})")
                                    st.write(f"**Segmentos**: {', '.join(sup['segmentos'])}")
                                    for j, aula in enumerate(sup['aulas']):
                                        turma = obter_turma_aula(aula)
                                        horario_num = obter_horario_aula(aula)
                                        st.write(f"  {j+1}. Turma: {turma}, Disciplina: {obter_disciplina_aula(aula)}, Período: {horario_num}º")
                        ...
                        if conflitos or limites_excedidos:
                            ...
                            if conflitos:
                                # Mostrar conflitos com horários REAIS
                        ...
                        if aulas:
                            st.subheader("🔍 DIAGNÓSTICO DA GRADE (COM HORÁRIOS REAIS)")
                            ...
                            if "CRÍTICO" in diagnostico['status']:
                                st.error("🚨 **ALERTA CRÍTICO**: Existem professores com aulas sobrepostas no mesmo horário REAL!")
                            ...
                            if (diagnostico['professores_saturados'] or 
                                ...
                                with st.expander("👨‍🏫 ANÁLISE DE PROFESSORES (COM HORÁRIOS REAIS)"):
                                    # Superposições (CRÍTICO) - com horários REAIS
                                    if diagnostico['professores_superpostos']:
                                        st.write("#### 🚨 **SUPERPOSIÇÕES CRÍTICAS**")
                                        for sup in diagnostico['professores_superpostos'][:3]:
                                            st.write(f"❌ **{sup['professor']}**: {len(sup['aulas'])} aulas às {sup['dia']}, {sup['horario_real']}")
                                            segmentos = ", ".join(set(sup['segmentos']))
                                            st.write(f"  Segmentos: {segmentos}")
                        ...
                        # ============================================
                        if aulas:
                            st.subheader("📅 Visualização da Grade Horária (COM HORÁRIOS REAIS)")
                            ...
                            for turma_nome in turmas_com_aulas:
                                ...
                                for periodo in periodos:
                                    ...
                                    table_html += f"<tr><td class='horario-cell'><strong>{horario_real}</strong><br><small>(Período {periodo}º)</small></td>"
                            ...
                            # Dataframe detalhado COM HORÁRIOS REAIS
                            ...
                            df_aulas = df_aulas.sort_values(["Turma", "Dia", "Período"])
                            st.subheader("📊 Lista Detalhada das Aulas (COM HORÁRIOS REAIS)")
...
# ============================================
# ABA GRADE POR PROFESSOR (ATUALIZADA COM HORÁRIOS REAIS)
# ============================================
# ============================================
# ABA GRADE POR PROFESSOR (COM VISUALIZAÇÃO EM CALENDÁRIO ÚNICO)
# ============================================
with abas[6]:
    ...
    if not st.session_state.get('aulas'):
        ...
        if professor_selecionado:
            ...
            if not aulas_professor:
                ...
                # ============================================
                # GRADE COMPLETA DO PROFESSOR (TODAS AS AULAS JUNTAS)
                # ============================================
                st.subheader("📅 Grade Semanal Completa")
                
                # Determinar horários totais (maior período entre todas as turmas)
                max_periodos = 0
                turmas_professor = set()
                for aula in aulas_professor:
                    turma = obter_turma_aula(aula)
                    if turma:
                        turmas_professor.add(turma)
                        segmento_turma = obter_segmento_turma(turma)
                        periodos_turma = 7 if segmento_turma == "EM" else 5
                        if periodos_turma > max_periodos:
                            max_periodos = periodos_turma
                ...
                # Construir tabela
                dias_semana = ["segunda", "terca", "quarta", "quinta", "sexta"]
                periodos = list(range(1, max_periodos + 1))
                
                table_html = """<table class='grade-professor-table'>
                <tr>
                <th style='width: 100px;'>Período / Horário</th>"""
                
                # Cabeçalho dos dias
                for dia in dias_semana:
                    table_html += f"<th>{dia.capitalize()}</th>"
                table_html += "</tr>"
                
                # Para cada período
                for periodo in periodos:
                    # Determinar horário REAL para exibição (usar o mais comum ou deixar genérico)
                    horario_display = ""
                    if periodo <= 5:  # Períodos que existem tanto EM quanto EF II
                        if periodo == 1:
                            horario_display = "07:00-07:50 / 07:50-08:40"
                        elif periodo == 2:
                            horario_display = "07:50-08:40 / 08:40-09:30"
                        elif periodo == 3:
                            horario_display = "08:40-09:30 / 09:50-10:40"
                        elif periodo == 4:
                            horario_display = "09:50-10:40 / 10:40-11:30"
                        elif periodo == 5:
                            horario_display = "10:40-11:30 / 11:30-12:20"
                    elif periodo == 6:
                        horario_display = "11:30-12:20"
                    elif periodo == 7:
                        horario_display = "12:20-13:10"
                    
                    table_html += f"<tr><td class='periodo-cell'><strong>{periodo}º</strong><br><small>{horario_display}</small></td>"
                    
                    for dia in dias_semana:
                        # Encontrar todas as aulas deste professor neste dia/periodo
                        aulas_no_horario = []
                        for aula in aulas_professor:
                            if (obter_dia_aula(aula) == dia and 
                                obter_horario_aula(aula) == periodo):
                                aulas_no_horario.append(aula)
                        
                        if aulas_no_horario:
                            # Verificar se tem superposição (conflito crítico)
                            if len(aulas_no_horario) > 1:
                                table_html += f"""<td class='conflito-cell aula-professor-cell'>
                                <div class='disciplina-professor'>⚠️ CONFLITO!</div>
                                <div class='turma-professor'>{len(aulas_no_horario)} aulas simultâneas</div>
                                """
                                
                                # Listar as aulas em conflito
                                for aula_conflito in aulas_no_horario:
                                    disc = str(obter_disciplina_aula(aula_conflito))[:15]
                                    turma = str(obter_turma_aula(aula_conflito))
                                    table_html += f"<div class='horario-professor'>{disc} - {turma}</div>"
                                
                                table_html += "</td>"
                            else:
                                # Apenas uma aula - mostrar normalmente
                                aula = aulas_no_horario[0]
                                disciplina = str(obter_disciplina_aula(aula)).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                                turma = str(obter_turma_aula(aula)).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                                horario_real = obter_horario_real(turma, periodo)
                                ...
                                table_html += f"""<td class='aula-professor-cell' style='background-color: {cor_fundo}; color: {cor_fonte};'>
                                <div class='disciplina-professor'>{disciplina}</div>
                                <div class='turma-professor'>{turma}</div>
                                <div class='horario-professor'>{horario_real}</div>
                                </td>"""
                        else:
                            table_html += "<td class='vazio-cell'>-</td>"
                    
                    table_html += "</tr>"
                    
                    # Adicionar linha do intervalo
                    if periodo == 3:
                        table_html += """<tr class='intervalo-row-professor'>
                        <td colspan='6'>
                        🕛 INTERVALO GERAL: 09:30 - 09:50
                        </td>
                        </tr>"""
                ...
                with col1:
                    st.write("**📋 Legenda:**")
                    st.write("• **Cores:** Representam as disciplinas")
                    st.write("• **⚠️ Vermelho:** Superposição crítica (professor em 2 lugares ao mesmo tempo)")
                    st.write("• **Horário:** Mostra o horário REAL da turma específica")
                
                with col2:
                    st.write("**📊 Resumo por Turma:**")
                    turmas_resumo = {}
                    for aula in aulas_professor:
                        turma = obter_turma_aula(aula)
                    ...
                    for turma, count in sorted(turmas_resumo.items()):
                        st.write(f"• **{turma}:** {count} aulas")
                
                # ============================================
                # LISTA DETALHADA (OPCIONAL - EM EXPANDER)
                # ============================================
                with st.expander("📋 Lista Detalhada das Aulas"):
                    # Criar dataframe
                    ...
                    if not df_professor.empty:
                        csv = df_professor.to_csv(index=False)
                        st.download_button(
                            "📥 Baixar Grade do Professor (CSV)",
                            csv,
                            f"grade_{professor_selecionado.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M')}.csv",
                            "text/csv"
                        )
                
                # ============================================
                # VERIFICAÇÃO DE CONFLITOS ESPECÍFICA
                # ============================================
                st.markdown("---")
                st.subheader("🔍 Verificação de Conflitos")
                
                # Verificar superposições específicas deste professor
                superposicoes_prof = []
                horarios_ocupados = {}
                
                for aula in aulas_professor:
                    dia = obter_dia_aula(aula)
                    horario_real = obter_horario_real(obter_turma_aula(aula), obter_horario_aula(aula))
                    chave = f"{dia}|{horario_real}"
                    
                    if chave not in horarios_ocupados:
                        horarios_ocupados[chave] = []
                    
                    horarios_ocupados[chave].append(aula)
                
                # Identificar conflitos
                for chave, aulas_conflito in horarios_ocupados.items():
                    if len(aulas_conflito) > 1:
                        dia, horario_real = chave.split('|')
                        superposicoes_prof.append({
                            'dia': dia,
                            'horario_real': horario_real,
                            'aulas': aulas_conflito,
                            'quantidade': len(aulas_conflito)
                        })
                
                if superposicoes_prof:
                    st.error(f"🚨 **{len(superposicoes_prof)} CONFLITO(S) DETECTADO(S)!**")
                    
                    for i, conflito in enumerate(superposicoes_prof, 1):
                        st.write(f"**Conflito {i}: {conflito['dia'].capitalize()}, {conflito['horario_real']}**")
                        for aula in conflito['aulas']:
                            st.write(f"  • {obter_disciplina_aula(aula)} - Turma {obter_turma_aula(aula)}")
                    
                    st.warning("⚠️ **Ação necessária:** Use o botão 'Corrigir Superposições' na aba Diagnóstico")
                else:
                    st.success("✅ Nenhum conflito detectado na grade deste professor!")
...
# ============================================
# ABA DIAGNÓSTICO (ATUALIZADA COM HORÁRIOS REAIS)
# ============================================
with abas[7]:
